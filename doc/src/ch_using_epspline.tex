\chapter{Using \dtypkgu{}}
	...
	in the following subsections.
		...
		the associated actions. Only the items that differ from
		common use or are unique to \IXpkg{} are described in
		more detail. Also, the accelerator keys that are shown
		may differ from those seen in a running instance of
		\IXpkg{}. The current windowing \IXarg{toolkit} or environment
		might have preferred accelerators for some common \IXarg{menu}
		items, in which case \IXpkg{} might use the preference.
			...
			\begin{description}
			  ...
			  in the empty tab with ``Open Here.''
			...
			\textsc{Item}: & \textsc{Keys}: & \textsc{Action}:\\ \hline
			\hline \hline
			Set Scale & --- & show a dialog to set arbitrary scale of drawing\\ \hline
			Cycle Scale & --- & cycle scale of drawing through normal, large, and small\\ \hline
			Toggle Quick Drawing & --- & switch between anti-aliased, and not, outlines\\ \hline
			Save Visible to File & --- & save the visible part of drawing area to image file\\ \hline
			\end{tabular}
			}%\textsf
			...
			Note on scaling: the scaling described above works
			nicely with the Microsoft Windows build (excepting the
			lossage at small scales, which is common to all),
			even when running under \IXnewterm{Wine} on
			\IXarg{\emph{GNU/Linux}}.
			Unfortunately, with the \emph{GTK2} builds of
			\IXarg{\emph{wxWidgets}} in versions 2.6 and 2.8,
			the display at scales other than 100\% is very slow,
			but \emph{only} with anti-aliased drawing. This is
			an inefficiency in the scaling of raster image data.
			Users of the \emph{GTK2} build might still find scaling
			useful if combined with ``quick drawing'' (see the
			``Toggle Quick Drawing'' item above). With the
			2.9 development version of \IXarg{\emph{wxWidgets}},
			the scaling of anti-aliased drawing (i.e., raster
			data) seems as fast as the Microsoft Windows build.
			...
			awkward when used for a simple preview. Nevertheless,
			...
			and might be stressed
			more strongly, since under Microsoft Windows temporary
			directories (usually per-user) don't seem to be cleaned
			on reboot, and accumulations might build.
