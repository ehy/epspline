...
\label{chap:using_epspline}
	...
	instance of \IXpkg{}.
		...
		\IXarg{points} with mouse clicks, as descibed
		so far, is just a rough start. If an idea of an
		object for a \IXpov{} \IXarg{scene} can be
		implemented as a \IXpov{} \IXarg{prism} or
		\IXarg{lathe}, then \IXpkg{} might help
		...
		provided by \IXpkg{} are discussed below.
			...
			it's created, it can be changed to a prism
			or lathe.
			...
			\begin{description}
			  \item[(label)] A label stating the type of spline.
			  ...
			  \item[Render Type] A selection of rendering options:
				as lathe, as prism, or ``Undefined,''
			  ...
			  \item[Sweep Type] This applies to prisms. The sweep
				...
				\href{\URLPOVdocsPrism}{prism documentation}.
				Note that presently a conic sweep prism is best
			  ...
			  \item[Use Sturm, Open] These are optional. Sturm selects
				...
				Open applies to the prism: the flat faces will not
				be rendered, leaving an open outline.
			  \item[Start, End Sweep] This applies to prisms:
				roughly, the offset and height of the prism.
			...
			will be necessary in any any scene, and some are
			discussed in \nameref{sec:useful_transforms}.
			\IXpkgu{} provides some interactive transforms
			that might be useful while developing objects.
			...
			\subsubsection{Adding and Deleting Points}%WX%5||||
